...
namespace Fluid.Core
{
    public class NetworkManager
    {
        ...
        public Stream BaseStream
        {
            ...
            set
            {
                lock (streamLock)
                {
                    ...
                    ConnectionStream = new ConnectionStream(BufferedStream);
                }
            }
        }

        private BufferedStream BufferedStream { get; set; }
        private ConnectionStream ConnectionStream { get; set; }
        ...
        public IPacket ReadPacket(PacketDirection direction)
        {
            ...
            {
                int idLength;
                long length = ConnectionStream.ReadVarInt();
                long id = ConnectionStream.ReadVarInt(out idLength);
                var data = ConnectionStream.ReadUInt8Array((int)(length - idLength));
                ...
                var ms = new ConnectionStream(new MemoryStream(data));
        ...
        public void WritePacket(IPacket packet, PacketDirection direction)
        {
            lock (streamLock)
            {
                var newNetworkMode = packet.WritePacket(ConnectionStream, NetworkMode, direction);
                ...
                ConnectionStream.WriteVarInt((int)BufferedStream.PendingWrites + ConnectionStream.GetVarIntLength(id));
                ConnectionStream.WriteVarInt(id);
